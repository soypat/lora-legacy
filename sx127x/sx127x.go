...
func New(spi drivers.SPI, cs, rstPin machine.Pin) *Device {
	k := Device{
		cs:     cs,
		spi:    spi,
		rstPin: rstPin,
...
func (d *Device) ReadRegister(reg uint8) uint8 {
	d.cs.Low()
	defer d.cs.High()
	err := d.spi.Tx([]byte{reg & 0x7f}, nil)
	if err != nil {
		println("spi error:", err.Error())
		return 0
	}
	var value [1]byte
	err = d.spi.Tx(nil, value[:])
	if err != nil {
		println("spi error:", err.Error())
		return 0
	}
...
func (d *Device) WriteRegister(reg uint8, value uint8) (uint8, error) {
	d.cs.Low()
	defer d.cs.High()
	// var buf [2 + 1]byte
	var response [1]byte
	err := d.spi.Tx([]byte{reg | 0x80}, nil)
	if err != nil {
		return 0, err
	}
	err = d.spi.Tx([]byte{value}, response[:])
	if err != nil {
		return 0, err
	}
...
func (d *Device) SetBandwidth(bw uint8) {
	d.loraConf.Bandwidth = bandwidth(bw)
...
func (d *Device) SetPreambleLength(pLen uint16) {
	d.loraConf.PreambleLength = pLen
...
func (d *Device) Tx(pkt []uint8, timeoutMs uint32) error {
	...
	d.SetPreambleLength(d.loraConf.PreambleLength)
	d.SetSyncWord(d.loraConf.SyncWord)
	d.SetBandwidth(d.loraConf.Bandwidth)
	d.SetSpreadingFactor(d.loraConf.Spread)
	d.SetIqMode(d.loraConf.IQ)
	d.SetCodingRate(d.loraConf.CodingRate)
